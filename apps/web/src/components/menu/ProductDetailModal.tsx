...
export default function ProductDetailModal({
  ...
  const [selectedModifiers, setSelectedModifiers] = useState<
    { name: string; options: { name: string; priceExtra: number }[] }[]
  >([]);
  ...
  const handleToggleOption = (
    modName: string,
    option: { name: string; priceExtra: number },
    type: string,
  ) => {
    setSelectedModifiers((prev) => {
      ...
      if (type === "single_select") {
        return prev.map((m) =>
          m.name === modName ? { ...m, options: [option] } : m,
        );
      }
      const modOptions = existingMod?.options || [];
      const isSelected = modOptions.some((o) => o.name === option.name);
      const newOptions = isSelected
        ? modOptions.filter((o) => o.name !== option.name)
        : [...modOptions, option];
      return prev.map((m) =>
        m.name === modName ? { ...m, options: newOptions } : m,
      );
    });
  };
  ...
  const calculateTotalPrice = () => {
    const modPrice = selectedModifiers.reduce((acc, mod) => {
      return (
        acc +
        mod.options.reduce((oAcc, opt) => oAcc + opt.priceExtra, 0)
      );
    }, 0);
            ...
            >
              <Dialog.Panel className="w-full max-w-5xl transform overflow-hidden bg-[#0A0A0A] sm:rounded-[40px] shadow-2xl transition-all flex flex-col md:flex-row max-h-[calc(100vh-5rem)] sm:max-h-[calc(100vh-4rem)] md:max-h-[min(90vh,880px)]">
                ...
                <div className="w-full md:w-[58%] flex flex-col overflow-hidden min-h-0">
                            ...
                            {mod.options.map((opt) => {
                              ...
                              return (
                                <button
                                  ...
                                  type="button"
                                  onClick={() =>
                                    handleToggleOption(mod.name, opt, mod.type)
                                  }
                                ...
                                >
                                  <div className="flex items-center gap-4">
                                    <div
                                      className={`h-4 w-4 rounded-full border-2 flex items-center justify-center ${isSelected ? "border-[#D4AF37]" : "border-gray-600"}`}
                                    >
                                      {isSelected && (
                  ...
                  <div className="p-6 md:p-8 border-t border-white/10 bg-black flex flex-col gap-6 shrink-0">
