...
export default function SportsTicker({ sportFilter = "ALL" }: SportsTickerProps) {
  const games = useQuery(api.sports_queries.getTodayGames);

  if (!games || games.length === 0) return null;

  let displayGames = games.filter(isTickerGame);
  if (sportFilter !== "ALL") {
    const forSport = displayGames.filter((g) => g.sport === sportFilter);
    displayGames = forSport.length > 0 ? forSport : displayGames;
  }
    ...
    <div className="bg-black border-y border-white/5 py-4 overflow-hidden relative group">
      ...
      <div className="flex animate-marquee-slow group-hover:pause">
        {marqueeGames.map((game, i) => (
          <div
            key={`${game._id}-${i}`}
            className="flex items-center gap-6 px-10 border-r border-white/5 whitespace-nowrap"
          >
            <span
              className={cn(
                "text-[#D4AF37] font-black text-[10px] uppercase tracking-widest px-2 py-0.5 border border-[#D4AF37]/30 rounded",
                sportFilter !== "ALL" && game.sport === sportFilter && "bg-[#D4AF37]/10",
              )}
            >
              {game.sport}
            </span>
            ...
            <div className="flex flex-col items-center">
              {game.status === "inprogress" ? (
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="text-red-600 font-black uppercase text-[9px] tracking-widest">
                    Live
                  </span>
                </div>
              ) : game.status === "closed" ? (
                <span className="text-gray-500 font-black uppercase text-[9px] tracking-widest">
                  Final
                </span>
              ) : (
