...
import {
  applyPrimeTimeFlags,
  applyRacingFeaturedFlags,
  applyUfcFeaturedFlags,
} from "./espn/sync";
...
export async function syncSportForDate(
  ...
  try {
    ...
    normalizedGames =
      sport === "UFC"
        ...
        : sport === "NASCAR" || sport === "F1"
          ? applyRacingFeaturedFlags(normalizedGames, sport)
  ...
  // ── SOURCE 2: API-Sports ──────────────────────────────────────────────────
  if (sport !== "GOLF" && sport !== "UFC" && sport !== "NASCAR" && sport !== "F1") {
    try {
      await sleep(300);
